...
/**
 * InventoryController is a controller class that acts as a "middleman" between
 * the view classes - {@link AdminView},  {@link InventoryView}, {@link DisplayMedicalInventoryView}
 * {@link ManageMedicalInventoryView} and {@link ManagReplenishmentRequestView}
 * and the model class - {@link Admin}, {@link InventoryList} and
 * {@link Medicine}.
 * <p>
 *
 * It can view/add/update/remove medical inventory items and get/approve/reject
 * replenishment requsts
 *
...
public class InventoryController {
    ...
    /**
     * Adds a new medicine to the medical inventory and persists the data to the
     * repository. Creates a new InventoryList item for the provided medicine
    ...
    /**
     * Removes a medical inventory item by its medicine ID. Prompts the user for
     * confirmation before removal and persists the changes to the repository.
     *
     * @param medicineId The unique identifier of the medicine to be removed.
     * @return {@code true} if the item was successfully removed, {@code false}
     * otherwise.
     */
     ...
     * @return The {@link InventoryList} item associated with the provided
     * medicine ID, or {@code null} if not found.
     */
    ...
    /**
     * Updates the stock level of a specific medical inventory item. Persists
     * the updated inventory to the repository.
     *
     * @param medicineId The unique identifier of the medicine to update.
     * @param stockLevel The new stock level for the medicine.
     * @return {@code true} if the stock level was successfully updated,
     * {@code false} if the medicine ID was not found.
     */
     ...
     * @return {@code true} if the alert level was successfully updated,
     * {@code false} if the medicine ID was not found.
     */
    ...
    /**
     * Retrieves all replenishment requests grouped by their status (Pending,
     * Approved, Rejected).
     *
    ...
    /**
     * Approves a replenishment request and updates the stock level of the
     * associated inventory item. This method will deny approval if the request
     * has already been approved or rejected.
     *
     * @param requestId The unique identifier of the replenishment request to
     * approve.
     * @return {@code true} if the request was successfully approved,
    ...
    /**
     * Rejects a replenishment request by updating its status to REJECTED. This
     * method will deny rejection if the request has already been approved or
     * rejected.
     *
     * @param requestId The unique identifier of the replenishment request to
     * reject.
     ...
     * @param requestId The unique identifier of the replenishment request to
     * retrieve.
     * @return The {@link ReplenishmentRequest} associated with the provided
     * request ID, or {@code null} if not found.
     */
    ...
    /**
     * Checks all inventory items and retrieves details of items. Includes an
     * option to filter items based on their stock status.
     *
     * @param all {@code 1} to list all items, {@code 0} to list only items with
     * low stock levels.
     ...
     *
     * @return A formatted string containing the details of all inventory items,
     * or "No inventory records found" if empty.
     */
